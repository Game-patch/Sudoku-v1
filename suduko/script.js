...
function provideHint() {
  ...
  if (emptyCells.length > 0) {
    ...
    // Disable the hint button if max hints are used
...
function showHintModal(message) {
  ...
  modal.classList.add("show"); // Fade-in effect
  const modalContent = modal.querySelector(".modal-content");
  modalContent.classList.add("show"); // Scale-up effect

  // Close modal after 3 seconds
  setTimeout(() => {
    modal.classList.remove("show"); // Fade-out effect
    modalContent.classList.remove("show"); // Scale-down effect
  }, 3000);
}
